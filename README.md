...
- **WaveAdamw.ipynb**  
  Benchmarks a custom optimizer (Wave-AdamW, applying row-wise Laplacian smoothing to AdamW) against standard AdamW on toy MLP (two-moons), CIFAR-10 CNN (ResNet-like, mixed precision), and AG News Transformer text
...
- **asl_augmentation.ipynb**  
  ...
  data augmentation techniques applied to an American Sign Language (ASL) image classification model to reduce overfitting

###  🌐 Computational Physics

  - **PINN_physicNN.ipynb** Tutorial on Physics-Informed Neural Networks (PINNs) with PyTorch.
...
- **quantum_randomness.ipynb**  
  ...
  Explores quantum randomness vs classical pseudo-random number generation. Implements a Monte Carlo estimation of pi using classical (linear congruential), quantum circuit-based (Hadamard gates on Quantum

- **qbits_coherence.ipynb**  
  Analysis of coherence in qubits, a fundamental concept in quantum computing.
  Simulates photon energy transfer inspired by photosynthesis using QuTiP. Models quantum walks, superposition/coherence in a photonic waveguide, light-harvesting complex interactions, time-crystal-like periodic
   driving, and decoherence dynamics.
